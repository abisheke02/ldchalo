...
const UpgradeModal = ({ sub, onClose, schoolId }) => {
  ...
  const handleCheckout = async () => {
    ...
    setLoading(true);
    try {
      const data = await paymentsAPI.createOrder({ schoolId, planType: selectedPlan, studentCount });
    ...
    <Layout>
      <div className="p-8">
        ...
        {loading ? (
          ...
          <div 
            title="Click 'New Class' to create a class and get started!"
            className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-main)] p-6 text-center max-w-md mx-auto card-shadow hover:shadow-md transition-shadow cursor-help"
          >
            <div className="flex items-center justify-center gap-4 mb-3">
              <span className="text-3xl">🏫</span>
              <div className="text-left">
                <h3 className="text-lg font-bold text-[var(--text-main)]">No classes yet</h3>
                <p className="text-[var(--text-muted)] text-sm">Create a class to get a join code.</p>
              </div>
            </div>
            <button
              onClick={() => setShowCreateModal(true)}
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            {classes.map((cls) => (
              <div
                key={cls.id}
                title={`View details for ${cls.class_name}`}
                onClick={() => navigate(`/classes/${cls.id}`)}
                className="bg-[var(--bg-card)] rounded-xl border border-[var(--border-main)] p-5 cursor-pointer card-shadow hover:shadow-lg hover:border-blue-300 transition-all group"
              >
                <div className="flex items-start justify-between mb-3">
                  <div>
                    <h3 className="font-bold text-[var(--text-main)] text-base group-hover:text-blue-600 transition">
                      {cls.class_name}
                    </h3>
