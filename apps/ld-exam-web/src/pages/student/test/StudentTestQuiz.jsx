    ...
    <div className="flex flex-col items-center gap-4 py-4">
      {micState === 'done' && transcript ? (
        ...
        <p className="text-sm text-slate-500">
          {micState === 'listening' ? 'Listeningâ€¦ speak now' : 'Tap the mic and speak your answer'}
        </p>
      )}
    ...
    return (
      <div className="flex items-center justify-center py-24">
        <div className="text-center">
          ...
          <p className="text-slate-500">Loading Level {level} testâ€¦</p>
        </div>
      </div>
    );
  }
  ...
  if (questions.length === 0) {
    return (
      <div className="text-center py-24">
        ...
        <button onClick={onBack} className="mt-4 text-blue-600 text-sm hover:underline">â† Back to levels</button>
      </div>
    );
  }
    ...
    <div className="max-w-2xl mx-auto px-4 py-6">
      ...
      {timeLeft < 120 && (
        <div className="bg-red-50 border border-red-200 rounded-xl px-4 py-2 mb-4 text-sm text-red-700 font-medium text-center">
          âš ï¸ Less than 2 minutes remaining!
        </div>
      )}
