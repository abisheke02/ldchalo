...
export default function StaffPage() {
  ...
  useEffect(() => {
    Promise.all([
      api.get('/school/staff', { params: { staff_type: filter || undefined } }),
      api.get('/school/staff/attendance/today'),
    ...
    <div className="space-y-5">
      ...
      {attendance.length > 0 && (
        <div className="bg-white rounded-xl shadow overflow-hidden">
          <div className="bg-primary text-white px-5 py-3 text-sm font-semibold">Staff Attendance â€” Today</div>
            ...
            <tbody>
              {staff.map((s, i) => (
                <tr key={s.id} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                  ...
                  <td className="px-4 py-2.5 text-gray-500">{s.phone || 'â€”'}</td>
                  ...
                  <td className="px-4 py-2.5 text-gray-500">{s.department || 'â€”'}</td>
                  <td className="px-4 py-2.5 font-mono text-xs">{s.employee_code || 'â€”'}</td>
                </tr>
              ))}
