...
export default function FeesPage() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    if (tab === 'Outstanding') {
      api.get('/school/fees/outstanding').then(r => { setData(r.data.rows); setTotals(r.data.totals); }).catch(() => toast.error('Failed')).finally(() => setLoading(false));
    } else if (tab === 'Concessions') {
      api.get('/school/fees/concessions').then(r => setData(r.data)).catch(() => toast.error('Failed')).finally(() => setLoading(false));
    } else {
        ...
        <>
            ...
            <div className="bg-white rounded-xl shadow overflow-hidden">
              {totals && (
                <div className="grid grid-cols-3 bg-primary text-white text-center py-3">
                  <div><p className="text-xs opacity-70">Total Fees</p><p className="font-bold">â‚¹{Number(totals.total_fees).toLocaleString()}</p></div>
                  <div><p className="text-xs opacity-70">Collected</p><p className="font-bold text-green-300">â‚¹{Number(totals.receipts).toLocaleString()}</p></div>
                  <div><p className="text-xs opacity-70">Outstanding</p><p className="font-bold text-amber-300">â‚¹{Number(totals.outstanding).toLocaleString()}</p></div>
                </div>
              )}
              <table className="w-full text-sm">
                <thead className="bg-gray-100">
                  <tr>{['Adm.No', 'Student', 'Class', 'Total (â‚¹)', 'Paid (â‚¹)', 'Outstanding (â‚¹)'].map(h => (
                ...
                <tbody>
                  {data.map((r, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-4 py-2">â‚¹{Number(r.total_fees).toLocaleString()}</td>
                      <td className="px-4 py-2 text-green-600">â‚¹{Number(r.receipts).toLocaleString()}</td>
                      <td className="px-4 py-2 text-red-600 font-semibold">â‚¹{Number(r.outstanding).toLocaleString()}</td>
                    </tr>
                  ))}
          ...
          {tab === 'Concessions' && (
            <div className="bg-white rounded-xl shadow overflow-hidden">
              <table className="w-full text-sm">
                <thead className="bg-gray-100">
                  <tr>{['Student', 'Class', 'Fee Head', 'Category', 'Term', 'Actual (â‚¹)', 'Concession (â‚¹)', '%'].map(h => (
                    <th key={h} className="px-4 py-2.5 text-left text-xs font-semibold text-gray-600">{h}</th>
                  ))}</tr>
                </thead>
                <tbody>
                  {data.map((r, i) => (
                    <tr key={i} className={i % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
                      ...
                      <td className="px-4 py-2">â‚¹{Number(r.actual).toLocaleString()}</td>
                      <td className="px-4 py-2 text-green-600">â‚¹{Number(r.concession_amount).toLocaleString()}</td>
              ...
              <div className="grid grid-cols-3 gap-4">
                {data.map(r => (
                  <div key={r.collection_method} className="bg-white rounded-xl shadow p-5">
                    ...
                    <p className="text-2xl font-bold text-gray-800 mt-1">â‚¹{Number(r.total_collected).toLocaleString()}</p>
