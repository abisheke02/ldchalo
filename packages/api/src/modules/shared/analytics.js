...
// â”€â”€â”€ GET /api/analytics/class/:classId/heatmap â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Returns student Ã— error-type grid for the class heatmap
...
// â”€â”€â”€ GET /api/analytics/class/:classId/at-risk â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
// Students whose 7-day avg score dropped â‰¥15% vs previous 7-day avg
...
// â”€â”€â”€ GET /api/analytics/student/me â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
router.get('/student/me', requireAuth, (req, res, next) =>
  getStudentAnalytics(req.user.userId, res, next)
);

// â”€â”€â”€ GET /api/analytics/student/:studentId â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
router.get('/student/:studentId', requireAuth, (req, res, next) =>
  getStudentAnalytics(req.params.studentId, res, next)
);

// â”€â”€â”€ GET /api/analytics/admin/overview â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
...
// GET /api/analytics/dashboard — school-level summary for mobile dashboard
router.get('/dashboard', requireAuth, requireRole('teacher', 'admin', 'school_admin'), async (req, res, next) => {
  try {
    ...
    const [studentsRes, attendanceRes, feesRes] = await Promise.all([
      query(
        `SELECT COUNT(*) as total, COUNT(CASE WHEN created_at >= NOW() - INTERVAL '90 days' THEN 1 END) as new_this_term
      ...
      query(
        `SELECT
           COALESCE(SUM(CASE WHEN fr.status = 'paid' THEN fr.amount ELSE 0 END), 0) as collected,
           COALESCE(SUM(fr.amount), 0) as total_due
         FROM fee_records fr
         JOIN users u ON u.id = fr.student_id
         WHERE u.school_id = $1`,
        [schoolId]
      ).catch(() => ({ rows: [{ collected: 0, total_due: 0 }] })),
    ]);
    ...
    const totalDue = Number(f.total_due);

    res.json({
      as_of: today,
      students: { total: Number(s.total), new_this_term: Number(s.new_this_term) },
      attendance: { present: Number(a.present), total: Number(a.total) },
      fees: {
        collected,
        outstanding: totalDue - collected,
        collection_pct: totalDue > 0 ? Math.round((collected / totalDue) * 100) : 0,
      },
    });
  } catch (err) {
    next(err);
