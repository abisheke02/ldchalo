...
router.post('/generate', authenticate, authorize('principal', 'school_admin'), async (req, res, next) => {
  try {
    ...
    const prompt = `You are a school timetable generator. Given the following teacher-subject-class rules, generate a weekly timetable (Monâ€“Fri, 8 periods/day) with NO conflicts (same teacher in two classes at the same time).
